...
{
    public partial class tableFav : UserControl
    {
        ...
        public tableFav(List<Anime> list, int x=0)
        {
            InitializeComponent();
            this.animeList = list;
            this.next.Visible = false;
            if (login.adminmode == true) status.Items.Add("Featured"); 
            X = x;
        }
        ...
        void Filter(int x = 0)
        {
            ...
            {
                if (selectedStatus == "Featured") final = retrieve.GetList(88, 0, "SELECT * FROM Animelist\r\nWHERE animeid IN (\r\n    SELECT animeid FROM Featured\r\n)\r\nOR animeid IN (\r\n    SELECT animeid FROM FeaturedMovies\r\n)\r\nOR animeid IN (\r\n    SELECT animeid FROM FeaturedOthers\r\n)\r\n");
                else final = final.Where(y => y.Status.Contains(selectedStatus)).ToList();
            }
